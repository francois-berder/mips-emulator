...
const CP0_REG_COUNT: usize = 32;

pub struct CPU {
    regs: [u32; CPU_REG_COUNT],
    hi: u32,
    lo: u32,
    pc: u32,
    epc: u32,
    cp0: [u32; CP0_REG_COUNT],
}
...
impl fmt::Display for CPU {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        for i in 0..=31 {
            writeln!(f, "r{}: {:#x}", i, self.get_reg(i))?;
        }
        writeln!(f, "hi: {:#x}", self.hi)?;
        writeln!(f, "lo: {:#x}", self.lo)?;
        writeln!(f, "pc: {:#x}", self.pc)?;
        writeln!(f, "epc: {:#x}", self.epc)
    }
}
...
pub fn decode_program(program: &[u8]) -> Vec<Instruction> {
    let instruction_count = program.len() / 4;
    let mut instructions: Vec<Instruction> = Vec::<Instruction>::with_capacity(instruction_count);

    for i in 0..instruction_count {
        let instruction: u32 = (program[i * 4] as u32)
            | ((program[i * 4 + 1] as u32) << 8)
            | ((program[i * 4 + 2] as u32) << 16)
            | ((program[i * 4 + 3] as u32) << 24);
...
pub fn execute_instruction(cpu: &mut CPU, mem: &mut [u8], inst: Instruction) {
    match inst {
        ...
        Instruction::Mfc0 { rt, rd } => {
            cpu.set_reg(rt, cpu.cp0[cpu.get_reg(rd) as usize]);
        }
        Instruction::Mtc0 { rt, rd } => {
            cpu.cp0[cpu.get_reg(rd) as usize] = cpu.get_reg(rt);
        }
        Instruction::Syscall => {
            cpu.epc = cpu.pc;
            cpu.pc = 0x3c - 4;
